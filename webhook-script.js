...
function _handleOrder(sheet, body) {
  ...
  // sees the literal "1" / "2/1/3" string instead of a Sheets-parsed
  // Date or Number — preventing the `Total quantity` ever drifting
  // into "Sat Jan 03 2026 00:00:00" territory.
 ...
 * slash-joined multi-value: SKU, Product name, Total quantity.
 *
 ...
 *
 *  Slot model — SKU / Product name / Total quantity are ALWAYS three
 *  slash-joined slots in this exact order:
 *    1. base       (cart lines added from PDP / hero / sugarbear flow)
 *    2. upsell     (post-purchase 99-SAR offer)
 *    3. cross_sell (in-cart cross-sell card additions)
 *
 *  Empty slots are still present — "" for SKU/name, "0" for quantity.
 *
 *  ORDER (base + cross-sell, no upsell yet):
 ...
 *    }
 *
 *  UPSELL  (updates the row above in place — replaces the MIDDLE slot):
 *    POST <web-app-url>?apiKey=YOUR_KEY
 *    Content-Type: application/json
 *
 *    {
 ...
 *    }
 *
 *  RESULT in the sheet (single row, after upsell):
 *    SKU             FN-SUG-004/FN-CREAM-002/FN-CREAM-002
 *    Product name    فيتامينات سوجاربير للشعر/كريم ترميم الحاجز/كريم ترميم الحاجز
 *    Total quantity  1/1/3
 *    Variant price   298
 *
 *  Legacy rows (single-segment "3" or two-segment "3/1") are padded
 *  on the first upsell merge so the post-merge value is always exactly
 *  three slots ("3/1/0", never "3/1").
 * ──────────────────────────────────────────────────────────────────────── */
