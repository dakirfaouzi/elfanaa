...
export function buildCreativePromptsSystemPrompt(opts: {
  ...
  return buildSystemPrompt({
    ...
    stageRules: [
      ...
      // Step 4 Phase 4.6.4b (round 2) — the #1 failure is collapsing every scene
      // into "an attractive person holding the product". The intent is now the
      // AUTHORITATIVE driver of composition, OVERRIDING any human-default below.
      "ASSET TYPE IS AUTHORITATIVE — each scene's `intent` DICTATES its composition (see the SHOT LIST) and the IMAGE ITSELF must visually communicate that section's job. Collapsing every scene into 'a person holding the product' is the SINGLE BIGGEST FAILURE and is forbidden. Specifically: `ingredient` = a product + raw-ingredient/texture MACRO still-life with NO model and NO face; `mechanism` = a tight APPLICATION close-up of hands using the product ON THE TARGET AREA (not a full portrait); `result` = the visible OUTCOME / improved target area (the 'after'), product secondary; `benefit` = the PROBLEM→SOLUTION on the relevant body area; only `hero`, `proof`, and `context` are person-led portraits/scenes.",
      "HUMAN PRESENCE applies ONLY to the person-led intents — `hero`, `result`, `proof`, `context` (full person) and as HANDS-ONLY for `mechanism` and `benefit` application. The `ingredient`/detail intent has NO human. NEVER force a full person or a posed portrait into an ingredient, mechanism, result, or benefit asset — those must foreground the texture / application / outcome / target-area, not a model.",
      ...
      "COMPOSITION SERVES THE SECTION: the product is always visible, but the FRAME is built around the intent's subject (texture / application / outcome / problem-area / customer / lifestyle), NOT around a model presenting the bottle. A section creative that is merely a model holding the product squared to camera is a FAILURE — build the marketing asset the section needs.",
      "CAST THE HUMAN FROM THE AUDIENCE DIRECTIVE (for the person-led intents only): match gender, age band, and market to the audience above (beauty→woman, men's grooming→man, family/home→a family or parent, wellness→an appropriate adult). When the market is GCC, people must read as authentically Gulf/Khaleeji — tasteful, modest, premium — never a generic Western stock look. When no audience is set, cast the product's most natural customer.",
      ...
      // Step 4 Phase 4.6.4b round 3 — scale/placement realism + text-light creatives.
      "PRODUCT SCALE & PLACEMENT REALISM: depict the product at its TRUE real-world size (a 30–60ml bottle/jar/tube is small in a hand and small within a wider scene). NEVER an oversized/giant product, NEVER a floating product, NEVER a product intersecting or merging with a face/skin/body. It must rest on a real surface or sit in a natural grip with believable contact shadows — a real photograph, not a pasted cut-out.",
      "THE IMAGE CARRIES THE MESSAGE: write each scene so the picture ALONE communicates its section (the texture, the application, the visible outcome, the problem-area) with little or no reliance on a caption — the copy underneath is support, not the payload. Benchmark direct-response creatives read at a glance.",
      ...
      "Section scenes: premium GCC e-commerce ADVERTISING photography (a high-end Gulf DTC brand), the EXACT product clearly visible, composed per the scene's `intent` (macro / application / outcome / problem-area / portrait / lifestyle — NOT a uniform 'person holding product'), mobile-first VERTICAL framing.",
      "Each prompt 60–120 words, dense with camera + sensory detail (lens, light, mood, environment).",
      "Provide a STRONG per-image `negative`: text, watermark, logo, deformed hands, extra fingers, fused fingers, plastic/waxy skin, uncanny face, lowres, jpeg artefacts, duplicated product, oversized/giant product, floating product, product intersecting the face/body, pasted cut-out.",
...
export function buildCreativePromptsUserPrompt(opts: {
  ...
  return [
    ...
    "its matching section. Each is a PURPOSE-BUILT marketing asset — write the prompt",
    "so the IMAGE ITSELF communicates the section. The composition recipe below is",
    "MANDATORY; the 'NOT' on each line is a hard failure:",
    "  • 'ingredient' — MACRO still-life: the EXACT product beside its hero ingredient",
    "                   + a texture swatch (oil droplets, cream smear, botanicals,",
    "                   powder) on a clean premium surface, shallow depth of field,",
    "                   tactile close-up. NO model, NO face, NO hands gripping. → ingredients",
    "  • 'mechanism'  — APPLICATION close-up ON THE TARGET AREA (face / under-eye /",
    "                   scalp / hairline / midsection — whatever this product treats):",
    "                   the customer's hand dispensing or applying the EXACT product to",
    "                   that area, cropped tight to the action. NOT a full-body portrait. → how-it-works",
    "  • 'result'     — the OUTCOME / 'after': the visibly improved target area or a",
    "                   radiant glow that proves the promise, product present but",
    "                   secondary. NOT a generic smiling model holding the bottle. → results",
    "  • 'benefit'    — PROBLEM→SOLUTION on the relevant body area: dramatise the",
    "                   concern being resolved (the relief / improvement), product",
    "                   present. NOT a generic lifestyle portrait. → benefits",
    "  • 'proof'      — TESTIMONIAL PORTRAIT: a confident, audience-matched real",
    "                   customer holding/using the product, direct eye contact. → social proof",
    "  • 'context'    — LIFESTYLE: the product at home in an aspirational GCC vanity /",
    "                   bathroom / dressing setting with the person. → lifestyle",
    "Always include 'ingredient', 'result', and 'proof'. Keep ONE coherent world",
    "(same person archetype where a person appears, same lighting, grade, surfaces,",
    "wardrobe) so the page reads as a single premium campaign — but vary the",
    "COMPOSITION per intent above; do NOT reuse 'person holding product' for all six.",
    "",
    "OUTPUT JSON:",
    "{",
